...
tx_dfs = [t_120, t_90, t_60, t_30, t_7]
...
tx_whdfs = [t_wh120, t_wh90, t_wh60, t_wh30, t_wh7]
...
tx_wh_all = tx_whdfs[0].join(tx_whjoins[0]).join(tx_whjoins[1]).join(tx_whjoins[2]).join(tx_whjoins[3]).fillna(0)
...
def depletion_estimator_6mons(df):
    ...
    for i in range(0,6):            
        x = np.where(last_3_mons_YoY.isnull() == False,
            ...
            t30)            
        estimates['sales_forecast: x months out',i] = x
...
test = sbc1_pred[['Description', 'Current_Vintage__c', 'Size__c', 'SBC_Cases_OH__c', 'NY_NJ_Committed_Cases__c',
            'NY_NJ_Available_Cases__c', 'OH_months_inventory',
            month_sbtrkt(-15), month_sbtrkt(-14), month_sbtrkt(-13), month_sbtrkt(-12), month_sbtrkt(-11),
            month_sbtrkt(-3), month_sbtrkt(-2), month_sbtrkt(-1),
             'last_3_months_YoY', 'cases_sold_t30',
        ('sales_forecast: x months out', 0),
       ('sales_forecast: x months out', 1),
       ('sales_forecast: x months out', 2),
       ('sales_forecast: x months out', 3),
       ('sales_forecast: x months out', 4),
       ('sales_forecast: x months out', 5)]]

test.head()

gc = gs.service_account(filename=setup.service_acct_key)
...
def reset_sheet(name):
    if name == 'Global':
        sheet_global.clear()
        gd.set_with_dataframe(sheet_global, global_pred)
    if name == 'NJ':
        sheet_nj.clear()
        gd.set_with_dataframe(sheet_nj, sbc1_pred)
    if name == 'CA':
        sheet_ca.clear()
        gd.set_with_dataframe(sheet_ca, caw1_pred)        
    if name == 'ILL':
        sheet_ill.clear()
        gd.set_with_dataframe(sheet_ill, ill1_pred)
    if name == 'VA':
        sheet_va.clear()
        gd.set_with_dataframe(sheet_va, vaw1_pred)
